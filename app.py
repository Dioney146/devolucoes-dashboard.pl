    ...
    if not _cols_ped:
        ...
        with bp1:
            busca_ped = st.text_input(
                "Buscar", placeholder="Nota, pedido, cliente, placa ou motivo",
                key="ped_busca", label_visibility="collapsed")
        with bp2:
            _ordem_opts = ["Maior valor", "Menor valor", "Nota de venda", "Cliente"]
            if COL_NUMPED:
        ...
        if _qtd_ped == 0:
            ...
            html_table(df_ped_view, min_width=1200)
            if _qtd_ped > 500:
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    panel_open("Sobre esta versão", tag="Interface v2.1", icon="ℹ️")
    st.markdown(
        ...
        'v2.1: novo painel "Pedidos com devolução" no Dashboard, com busca, ordenação e exportação em CSV.<br>'
