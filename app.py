...
(function removeBadge() {
  const selectors = [
    ...
    '.styles_viewerBadge__1yB5_','div[class*="StatusWidget"]',
        ...
        if motivo_reent_col2 and motivo_reent_col2 in df_r.columns:
            ...
            for i, row in df_rkr.iterrows():
                ...
                rows_html_r += f"""<tr style="background:{bg};">
                  ...
                  <td style="padding:10px 14px;color:#f59e0b;font-size:0.84rem;text-align:center;border-bottom:1px solid rgba(74,222,128,0.07);">{row['% Total']}</td>
                </tr>"""
            st.markdown(f"""
            <div style="background:rgba(6,13,31,0.92);border:1px solid rgba(74,222,128,0.15);
              border-radius:14px;overflow:hidden;max-height:360px;overflow-y:auto;">
              <table style="width:100%;border-collapse:collapse;">
                <thead><tr style="background:rgba(12,26,58,0.98);position:sticky;top:0;">
                  <th style="padding:12px 14px;color:#4ade80;font-size:0.75rem;font-weight:700;letter-spacing:0.08em;text-align:left;text-transform:uppercase;border-bottom:1px solid rgba(74,222,128,0.2);">Motivo</th>
                  <th style="padding:12px 14px;color:#4ade80;font-size:0.75rem;font-weight:700;letter-spacing:0.08em;text-align:center;border-bottom:1px solid rgba(74,222,128,0.2);">Qtd</th>
                  <th style="padding:12px 14px;color:#4ade80;font-size:0.75rem;font-weight:700;letter-spacing:0.08em;text-align:center;border-bottom:1px solid rgba(74,222,128,0.2);">% Total</th>
                </tr></thead>
                <tbody>{rows_html_r}</tbody>
              </table>
            </div>""", unsafe_allow_html=True)


# ══════════════════════════════════════════════════════════════
# ABA 3 — DETALHES DAS REENTREGAS
# ══════════════════════════════════════════════════════════════
with tab_reent_det:
    st.markdown('<div class="sec-header"><div class="bar"></div><h3>🔍 Campos das Reentregas — Pesquisa Detalhada</h3></div>', unsafe_allow_html=True)

    if df_reent is None or len(df_reent) == 0:
        st.warning("⚠️ Nenhum dado de reentregas disponível.")
    else:
        # Filtro de data específico para esta aba
        ...
        with det_f1:
            ...
            col_label_det = dt_col_det if dt_col_det else "DATATRANSF"
            if len(datas_det_ok) > 0:
                ...
                sel_det_data_str = st.selectbox(f"📅 Data de Transferência ({col_label_det})", opcoes_det, key="det_dtsel")
                if sel_det_data_str != "— Todas as datas —":
                    dt_det_sel = datetime.strptime(sel_det_data_str, "%d/%m/%Y").date()
                    usar_data_det = True
        ...
        # Aplicar filtro de data base para detalhes
        ...
        if usar_data_det and dt_det_sel:
            st.info(f"📅 Filtro de data ativo: {dt_det_sel.strftime('%d/%m/%Y')} (coluna {col_label_det}) — **{len(df_det_base)} registros**")

        # Pesquisa adicional
        ds1, ds2, ds3, ds4 = st.columns(4, gap="medium")
        with ds1:
            s_cli_r = st.text_input("👤 Cliente", placeholder="Nome do cliente", key="det_cli")
        with ds2:
            s_nf_r = st.text_input("📄 Nota Fiscal", placeholder="Nº da nota", key="det_nf")
        with ds3:
            s_ped_r = st.text_input("📦 Pedido", placeholder="Nº do pedido", key="det_ped")
        with ds4:
        ...
        df_det = df_det_base[[orig for orig, _ in cols_det_ok]].copy()
        ...
        if s_nf_r.strip() and "Nota Fiscal" in df_det.columns:
            df_det = df_det[df_det["Nota Fiscal"].str.contains(s_nf_r.strip(), case=False, na=False)]
        ...
        if len(df_det) == 0:
            ...
            col_headers_det = list(df_det.columns)
            thead_det = "".join([
                f'<th style="padding:11px 13px;color:#4ade80;font-size:0.74rem;font-weight:700;'
                f'letter-spacing:0.07em;text-align:left;text-transform:uppercase;white-space:nowrap;'
                f'border-bottom:1px solid rgba(74,222,128,0.22);background:rgba(10,26,48,0.99);">{c}</th>'
                for c in col_headers_det
            ])
            rows_det_html = ""
            for idx, (_, row) in enumerate(df_det.head(500).iterrows()):
                ...
                cells = "".join([
                    ...
                    f'border-bottom:1px solid rgba(74,222,128,0.05);white-space:nowrap;">{val}</td>'
                    for val in row.values
                ])
                rows_det_html += f'<tr style="background:{bg};">{cells}</tr>'

            st.markdown(f"""
            <div style="background:rgba(5,11,28,0.94);border:1px solid rgba(74,222,128,0.16);
              border-radius:14px;overflow:hidden;max-height:560px;overflow-y:auto;overflow-x:auto;
              box-shadow:0 4px 24px rgba(0,0,0,0.4);">
              <table style="width:100%;border-collapse:collapse;min-width:1100px;">
                <thead><tr>{thead_det}</tr></thead>
                <tbody>{rows_det_html}</tbody>
              </table>
            </div>""", unsafe_allow_html=True)

            if len(df_det) > 500:
                st.caption(f"⚠️ Exibindo primeiros 500 de {len(df_det)} registros. Use exportação para ver todos.")
...
with tab_campos:
    ...
    CAMPOS = [
        ...
        (COL_DTENTREGA,  "Data Entrega (DTENT)"),
    ...
    if len(df_campos) == 0:
        ...
        if len(df_campos) > 500:
            st.caption(f"⚠️ Exibindo primeiros 500 de {len(df_campos)} registros.")
